...
#        fprofit = `((sale*.93-total)>0)?(left>0)?((sale*.93-total)*.8+left*.5):((sale*.93-total)*.8):(sale*.93-total)`
    ...
    addActualLineitem: (e) ->
        ...
        jsRoutes.controllers.Lineitems.addActualLineitem().ajax
            data:
...
#                _view.el.find(".groupName").editInPlace("edit")
    ...
    completeTask: (e) ->
        ...
        target.parent().parent().find(".calTask").each (i,t) ->
            jsRoutes.controllers.Tasks.updateTask($(t).attr("data-task-id")).ajax
                data:
...
class Task extends Backbone.View
    ...
    editTask: (e) ->
        ...
        managerRow = $("<tr/>").append("<td>Manager</td>").append($("<td/>").load(jsRoutes.controllers.Home.getUserSelect().url,managerFunction))
        crewRow = $("<tr/>").append("<td>Crew</td>")
    ...
    deleteTask: (e) ->
        ...
        if (confirm('Are you sure?'))
            jsRoutes.controllers.Tasks.deleteTask(@el.attr("data-task-id")).ajax
                context: this
                success: (data) ->
                    context.el.remove()
                    for task in data
...
$ ->
    ...
    $("#jobCategory").live "change", (e) ->
        ...
        jsRoutes.controllers.Jobs.updateJob().ajax
            data:
                id: $("#totals").attr("data-job-id")
    ...
    $("#copyToPlans").click ->
        ...
        $("#dialog").dialog
            ...
            buttons: [
                text: "save"
                click: ->
                    jsRoutes.controllers.Jobs.copyToPlans().ajax
                        data:
                            id: $("#totals").attr("data-job-id")
    ...
    $("#applyTemplate").click ->
        ...
        $("#dialog").dialog
            ...
            buttons: [
                text: "apply"
                click: ->
                    jsRoutes.controllers.Jobs.applyTemplate().ajax
                        data:
                            id: $("#totals").attr("data-job-id")
        ...
        $("#dialog").html("").load(jsRoutes.controllers.Jobs.getPlanSelect().url,
    ...
    $("#jobMarket").dblclick (e) ->
        target = $(e.currentTarget)
        oldVal = target.html()
        jsRoutes.controllers.Jobs.getMarketSelect().ajax
            success: (data) ->
