...
class Plan extends Backbone.View
    ...
    addPlanItem: (e) ->
        ...
        jsRoutes.controllers.Plans.addPlanItem().ajax
            data:
...
class Attachments extends Backbone.View
    ...
    deleteAttachment: (e) ->
        ...
        if (confirm('Are you sure?'))
            jsRoutes.controllers.Plans.deleteAttachment(id).ajax
                success: ->
...
class PlanItem extends Backbone.View
    ...
    newDimension: (e) ->
        e.preventDefault()
        target = $(e.currentTarget)
        jsRoutes.controllers.Plans.addDimension().ajax
            context: this
            data:
...
class Dimension extends Backbone.View
    ...
    deleteDimension: (e) ->
        e.preventDefault()
        if (confirm('Are you sure?'))
            jsRoutes.controllers.Plans.deleteDimension(@id).ajax
                context: this
                success: ->
                    @length_feet.val(0)
                    @length_inches.val(0)
    ...
    updateDimension: (e) ->
        ...
        jsRoutes.controllers.Plans.updateDimension(@id).ajax
            context: this
            data: temp
            success: ->
...
$ -> # document is ready!
    ...
    $("#duplicatePlan").click ->
        ...
        newPlanCustomer = $("<td/>").load jsRoutes.controllers.Jobs.getCustomerSelect().url, ->
    ...
    $("#planName").live "dblclick", (e) ->
        ...
        input = $("<input/>").val(oldVal).blur ->
            ...
            if (context.val()!=oldVal)
                jsRoutes.controllers.Plans.updatePlan().ajax
                    data:
