...
public class Account extends Controller {
    ...
    public static Result changePassword() {
        ...
        if (!u.emailValidated) {
            return ok(views.html.account.unverified.render(u));
        } else {
            return ok(views.html.account.password_change_rd.render(PASSWORD_CHANGE_FORM, u));
        }
    }


    @Restrict(@Group(Application.USER_ROLE))
    public static Result changePassword_rd() {
        com.feth.play.module.pa.controllers.Authenticate.noCache(response());
        final User u = Application.getLocalUser(session());

        if (!u.emailValidated) {
            return ok(unverified.render(u));
        } else {
            return ok(password_change_rd.render(PASSWORD_CHANGE_FORM, u));
        }
    }
    ...
    public static Result doChangePassword() {
        ...
        if (filledForm.hasErrors()) {
            ...
            return badRequest(password_change.render(filledForm, Application.getLocalUser(session())));
        } else {
            final User user = Application.getLocalUser(session());
    ...
    @Restrict(@Group(Application.USER_ROLE))
    public static Result doChangePassword_rd() {
        com.feth.play.module.pa.controllers.Authenticate.noCache(response());
        final Form<Account.PasswordChange> filledForm = PASSWORD_CHANGE_FORM
                .bindFromRequest();
        if (filledForm.hasErrors()) {
            // User did not select whether to link or not link
            return badRequest(password_change_rd.render(filledForm, Application.getLocalUser(session())));
        } else {
            final User user = Application.getLocalUser(session());
            final String newPassword = filledForm.get().password;
            user.changePassword(new MyUsernamePasswordAuthUser(newPassword),
            ...
            flash(Application.FLASH_MESSAGE_KEY,
                    Messages.get("playauthenticate.change_password.success"));
            return redirect(routes.Application.profile_rd());
        }
    }
