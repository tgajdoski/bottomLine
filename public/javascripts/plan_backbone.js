$(window).on('load', function() {
    $('#planbudgetlists').DataTable().destroy();
...
$(document).ready(function() {
    ...
    $("#accordion").accordion({ active: 0});
        ...
        $("body").on('click','#planbudgetlists tbody tr', function(event) {
        if (!(event.target.tagName =="OPTION" || event.target.id == "itemSelect" ||  event.target.id == "vendorSelect" || event.target.className == "item" ||  event.target.className == "vendor" || event.target.className == "quantity" || event.target.className == "rate"))
        {
